...
from app.lib.bot import HCaptchaBot, CleanupWorker
...
class TestCleanupWorker(BaseCase):
    def setUp(self):
        ...
        self.test_worker = CleanupWorker(self.test_bot, self.test_app, hours=0)
    ...
    def test_run_simpe(self):
        ...
        message = Message(chat_id="2", message_id="2")
        db.session.add(message)
        db.session.commit()

        self.test_worker._cleanup()
        self.test_bot.delete_message.assert_called()
        self.test_bot.delete_message.assert_called_with(chat_id="2", message_id="2")
        m = Message.query.filter_by(chat_id="2", message_id="2").one_or_none()
        assert m is None

    def test_run_badrequest(self):
        """ Delete a message that is deleted on the telegram api <- BadRequest """
        # Add a message to the db
        message = Message(chat_id="3", message_id="3")
        db.session.add(message)
        db.session.commit()
        ...
        with patch.object(self.test_bot, "delete_message", _bot_delete_message_badrequest):
            ...
            self.test_bot.delete_message.assert_called()
            self.test_bot.delete_message.assert_called_with(chat_id="2", message_id="2")
            m = Message.query.filter_by(chat_id="3", message_id="3").one_or_none()
            assert m is None
