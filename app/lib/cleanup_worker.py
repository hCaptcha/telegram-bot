...
class CleanupWorker:
    ...
    def __init__(self, bot, app=None, minutes=None):
        ...
        self.minutes = (
            ...
            else int(get_active_config().CLEANUP_PERIOD_MINUTES)
        )
        ...
        with self.app.app_context():
            messages = (
                ...
                .filter(
                    ...
                    <= (datetime.now() - timedelta(minutes=self.minutes))
                )
                .all()
            )
            logger.info(f"Start to cleaning up {len(messages)} message...")
            deleted = 0
            for message in messages:
                deleted += self._cleanup_message(
                    message.id, message.chat_id, message.message_id
                )

            logger.info(f"Cleaning up {deleted} message done.")
    ...
    def _cleanup_message(self, _id, chat_id, message_id):
        ...
        with self.app.app_context():
            must_delete = False
            try:
                must_delete = self.bot.delete_message(
                    chat_id=chat_id, message_id=message_id
                )
            # Skip if the message not found for any reason
            except BadRequest as e:
                logger.debug(f"While deleting a bot message: {e}")
                must_delete = True
    ...
    def cleanup_all_user_messages(self, chat_id, user_id):
        ...
        with self.app.app_context():
            messages = (
                db.session.query(Message)
                .filter(Message.user_id == user_id, Message.chat_id == chat_id)
                .all()
            )
            logger.info(
            ...
            logger.info(f"Cleaning up {deleted} message done for user_id({user_id}).")
